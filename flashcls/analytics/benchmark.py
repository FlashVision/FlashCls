"""Benchmark — measure FlashClassifier model speed, size and parameter count."""
...
class Benchmark:
    """Benchmark FlashClassifier model speed and resource usage.

    Parameters
    ----------
    model_path : str | Path
        Path to a saved checkpoint (``.pth`` / ``.onnx``).
    device : str
        ``"cuda"`` or ``"cpu"``.
    ...
    def __init__(self, model_path: Union[str, Path], device: str = "cuda",
        ...
        self._is_onnx = self.model_path.suffix.lower() == ".onnx"
    ...
    def _bench_pytorch(self, model, dummy, warmup, iterations):
        ...
        return {"fps": round(fps, 2), "latency_ms": round(latency_ms, 3),
                "params": params, "model_size_mb": round(size_mb, 2)}
    ...
    def _bench_onnx(self, session, dummy, warmup, iterations):
        ...
        size_mb = self.model_path.stat().st_size / (1024 * 1024)
        return {"fps": round(fps, 2), "latency_ms": round(latency_ms, 3),
                "params": 0, "model_size_mb": round(size_mb, 2)}
